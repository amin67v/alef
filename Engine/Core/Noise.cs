...
namespace Engine
{
    ...
    public static class Noise
    {
        ...
        public static float Perlin2D(float x, float y)
        {
            ...
            var a = (perm[xi] + yi) & 0xff;
            var b = (perm[xi + 1] + yi) & 0xff;

            var grad_a = grad2D(perm[a], x, y);
            var grad_b = grad2D(perm[b], x - 1, y);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static float grad1D(int hash, float x) => (hash & 1) == 0 ? x : -x;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static float grad2D(int hash, float x, float y) => ((hash & 1) == 0 ? x : -x) + ((hash & 2) == 0 ? y : -y);
    }
}